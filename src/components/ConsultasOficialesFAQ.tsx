...
export default function ConsultasOficialesFAQ() {
    ...
    <section className="py-20 bg-white" aria-labelledby="faq-heading">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div data-sr className="text-center mb-12">
          <h2 id="faq-heading" className="text-3xl md:text-4xl font-black text-[#02132D] mb-4">
            Preguntas frecuentes
          </h2>
        </div>

        <div className="rounded-2xl overflow-hidden border border-gray-100">
          {consultasOficialesFaqItems.map((item, index) => {
            const isOpen = openItems.has(item.id);
            const isLast = index === consultasOficialesFaqItems.length - 1;
            return (
              <div key={item.id} className={isLast ? "" : "border-b border-gray-100"}>
                <button
                  ...
                  aria-controls={`consultas-faq-answer-${item.id}`}
                  id={`consultas-faq-question-${item.id}`}
                >
                  <span
                ...
                <div
                  id={`consultas-faq-answer-${item.id}`}
                  role="region"
                  aria-labelledby={`consultas-faq-question-${item.id}`}
