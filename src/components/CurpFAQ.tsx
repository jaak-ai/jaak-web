...
export default function CurpFAQ() {
  ...
  return (
    <section className="py-20 bg-white" aria-labelledby="curp-faq-heading">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div data-sr className="text-center mb-12">
          <h2 id="curp-faq-heading" className="text-3xl md:text-4xl font-black text-[#02132D] mb-4">
            Preguntas frecuentes
          </h2>
        </div>

        <div className="rounded-2xl overflow-hidden border border-gray-100">
          {curpFaqItems.map((item, index) => {
            const isOpen = openItems.has(item.id);
            const isLast = index === curpFaqItems.length - 1;
            return (
              <div key={item.id} className={isLast ? "" : "border-b border-gray-100"}>
                <button
                  ...
                  aria-controls={`curp-faq-answer-${item.id}`}
                  id={`curp-faq-question-${item.id}`}
                >
                  <span
                ...
                <div
                  id={`curp-faq-answer-${item.id}`}
                  role="region"
                  aria-labelledby={`curp-faq-question-${item.id}`}
