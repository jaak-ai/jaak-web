...
const LIVE_COMMENTS = [
  { id:  1, initials: "MR", name: "Miguel Rodríguez",  company: "HSBC México",        color: "#2DB6C1", text: "Excelente punto sobre la convergencia regulatoria entre banca y fintech 👏" },
  { id:  2, initials: "LC", name: "Laura Castillo",    company: "Kueski",              color: "#5A9EDB", text: "¿El modelo de neobanco aplica también para SOFOMES reguladas?" },
  { id:  3, initials: "JH", name: "Jorge Hernández",   company: "BBVA México",         color: "#7C6EDB", text: "El tema de PLD y liveness detection es muy relevante para nuestro equipo de cumplimiento" },
  { id:  4, initials: "AP", name: "Ana Pérez",         company: "Banorte",             color: "#4FC89A", text: "Muy claro el punto de quiebre que explica Arianna 🙌" },
  { id:  5, initials: "CV", name: "Carlos Vega",       company: "Clip",                color: "#E8956B", text: "¿Cuánto tarda en promedio la integración con un core bancario legacy?" },
  { id:  6, initials: "SM", name: "Sofía Morales",     company: "Nu México",           color: "#2DB6C1", text: "Perfecto el enfoque sobre NOM-151, gracias por cubrirlo 🙏" },
  { id:  7, initials: "ER", name: "Eduardo Ríos",      company: "Konfío",              color: "#5A9EDB", text: "La identidad sintética es el mayor reto que enfrentamos actualmente 🔥" },
  { id:  8, initials: "PL", name: "Patricia López",    company: "Santander México",    color: "#7C6EDB", text: "¿Tienen datos sobre reducción de fraude post-implementación KYC biométrico?" },
  { id:  9, initials: "DG", name: "Daniel García",     company: "Mercado Pago",        color: "#4FC89A", text: "El Dr. Merino explica perfecto el marco GAFI aplicado a México 💡" },
  { id: 10, initials: "IG", name: "Itzel González",    company: "AFORE XXI Banorte",   color: "#E8956B", text: "¿La NOM-151 es requisito obligatorio para contratos de AFORE también?" },
  { id: 11, initials: "RV", name: "Roberto Vargas",    company: "Stori",               color: "#2DB6C1", text: "Ya compartí este webinar con todo mi equipo de compliance 🔗" },
  { id: 12, initials: "FT", name: "Fernanda Torres",   company: "CIBanco",             color: "#5A9EDB", text: "El onboarding sin fricción es el reto más grande en nuestra SOFIPO" },
  { id: 13, initials: "AM", name: "Alejandro Martínez",company: "Banregio",            color: "#7C6EDB", text: "¿Aplica el mismo estándar de liveness para entidades IFPE?" },
  { id: 14, initials: "VR", name: "Valeria Ramírez",   company: "Spin by OXXO",        color: "#4FC89A", text: "Exactamente lo que necesitábamos para presentar al board 👍" },
  { id: 15, initials: "HG", name: "Hugo Guerrero",     company: "Nacional Financiera", color: "#E8956B", text: "Bien diferenciados los tres modelos: banca, fintech y neobanco 💡" },
  { id: 16, initials: "NF", name: "Nadia Fuentes",     company: "Crédito Real",        color: "#2DB6C1", text: "¿Tienen caso de éxito en SOFOM con volúmenes altos de originación?" },
  { id: 17, initials: "GO", name: "Gabriela Ochoa",    company: "Rappi Pay",           color: "#5A9EDB", text: "El punto de la firma electrónica en procesos de crédito es clave 🔑" },
  { id: 18, initials: "LS", name: "Luis Sánchez",      company: "Grupo Financiero Bx+",color: "#7C6EDB", text: "Muy completo el webinar, recomendado para cualquier equipo de compliance" },
];

/* ── Speaker accordion data ──────────────────────────────────────────── */
...
export default function WebinarPage() {
  ...
  useEffect(() => {
    if (localStorage.getItem("webinar-reg") === "1") {
      setRegistered(true);
    }
  }, []);

  /* Stream comments after registration */
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setTimeout(() => {
          document.getElementById("video-webinar")?.scrollIntoView({ behavior: "smooth" });
        }, 900);
      } else {
        setStatus("error");
      }
  ...
  return (
    <div data-hp-theme={light ? "light" : "dark"}>
      ...
      >
        ...
        <div className="relative z-10 max-w-6xl mx-auto w-full grid lg:grid-cols-[1fr_420px] gap-14 items-start">
            ...
            <div className="flex flex-wrap gap-2.5 mb-7">
              ...
              >
                <div className="w-8 h-8 rounded-full overflow-hidden shrink-0 relative">
                  <Image src="/images/webinar/arianna-quezada.jpg" alt="Arianna Quezada" fill className="object-cover object-top" onError={() => {}} />
                  <div className="absolute inset-0 flex items-center justify-center text-[10px] font-black" style={{ background: "linear-gradient(135deg,#25969f,#2DB6C1)", color: "#0E1133" }}>AQ</div>
                </div>
                <div>
                  <span className="block text-[11px] font-bold" style={{ color: "var(--hp-text-hi)", fontFamily: "var(--font-montserrat), Montserrat, sans-serif" }}>Arianna Quezada</span>
                  <span className="block text-[10px]" style={{ color: "var(--hp-text-lo)" }}>CEO &amp; Cofundadora · JAAK</span>
                </div>
              </div>
              <div
                className="flex items-center gap-2 rounded-full px-3 py-1.5 pr-3.5"
              ...
              >
                <div className="w-8 h-8 rounded-full overflow-hidden shrink-0 relative">
                  <Image src="/images/webinar/david-merino.jpg" alt="Dr. David Merino" fill className="object-cover object-top" onError={() => {}} />
                  <div className="absolute inset-0 flex items-center justify-center text-[10px] font-black text-white" style={{ background: "linear-gradient(135deg,#142654,#1E3A7A)" }}>DM</div>
                </div>
                <div>
                  <span className="block text-[11px] font-bold" style={{ color: "var(--hp-text-hi)", fontFamily: "var(--font-montserrat), Montserrat, sans-serif" }}>Dr. David Merino</span>
                  <span className="block text-[10px]" style={{ color: "var(--hp-text-lo)" }}>Especialista en Compliance · GAFI · ONU</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {heroTags.map((tag) => (
                <span
                  ...
                  style={{ color: "rgba(45,182,193,.85)", background: "rgba(45,182,193,.08)", border: "1px solid rgba(45,182,193,.2)" }}
                >
                  {tag}
                </span>
              ))}
          ...
          >
            ...
            <h2 className="text-lg font-black leading-snug mb-1.5 text-[#212A45]" style={{ fontFamily: "var(--font-montserrat), Montserrat, sans-serif" }}>
            ...
            {status === "success" ? (
              <div className="text-center py-6">
                ...
                <p className="text-sm text-gray-500 leading-relaxed mb-5">
                  Desplázate hacia abajo para ver el webinar completo con chat en vivo.
                </p>
                <a
                  href="#video-webinar"
                  ...
                  <div className="flex">
                    {["M", "R", "A"].map((l, i) => (
                      <span
                        ...
                        style={{ background: i === 0 ? "#2DB6C1" : i === 1 ? "#5A9EDB" : "#7EC8C2", color: i === 0 ? "#0E1133" : "#fff", zIndex: 3 - i }}
                      >
                        {l}
                      </span>
                    ))}
      ...
      <section
        id="video-webinar"
        style={{ borderTop: "1px solid var(--hp-divider)" }}
      >
        {registered ? (
          /* ── UNLOCKED: video + live chat ── */
          <div>
            {/* Live bar */}
            <div className="bg-[#080d1e] px-4 sm:px-6 py-2.5 flex items-center justify-between gap-4 flex-wrap">
              ...
              <div className="flex items-center gap-1.5">
                <svg viewBox="0 0 12 12" fill="currentColor" className="w-2.5 h-2.5 text-white/30">
            ...
            <div className="bg-black grid lg:grid-cols-[1fr_320px]">
              ...
              {/* Live comments panel */}
              <div
                className="flex flex-col bg-[#0D1128]"
                style={{ borderLeft: "1px solid rgba(255,255,255,.07)", minHeight: "320px" }}
              >
                {/* Panel header */}
                    ...
                    <div key={c.id} className="flex gap-2.5 animate-fade-in-up">
                      ...
                      <div className="min-w-0">
                        <div className="flex items-baseline gap-1.5 flex-wrap mb-0.5">
                          ...
                          <span className="text-[10px] text-white/28 truncate">{c.company}</span>
                        </div>
                ...
                {/* Input (decorative) */}
                <div className="p-3 shrink-0" style={{ borderTop: "1px solid rgba(255,255,255,.07)" }}>
                  <div
                    className="flex gap-2 items-center px-3 py-2.5 rounded-lg cursor-not-allowed"
                    style={{ background: "rgba(255,255,255,.05)", border: "1px solid rgba(255,255,255,.08)" }}
                    title="Chat disponible durante el webinar en vivo"
                  >
                    <span className="text-xs text-white/25 flex-1 select-none">Escribe un mensaje…</span>
                    <svg viewBox="0 0 16 16" fill="none" className="w-4 h-4 text-white/15 shrink-0">
                      <path d="M14 8H2M14 8l-4-4M14 8l-4 4" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                  </div>
                </div>
              </div>
          ...
          /* ── LOCKED: teaser with registration CTA ── */
            ...
            <div className="relative z-10 max-w-3xl mx-auto text-center">
              ...
              <div className="mb-8 mx-auto max-w-2xl">
                <div
                  ...
                  style={{ border: "1px solid rgba(45,182,193,.2)" }}
                >
                  {/* Background */}
                  <div className="absolute inset-0 bg-gradient-to-br from-[#0A1628] to-[#141a3a] flex items-center justify-center">
                    <p className="text-5xl font-black text-white/5 select-none leading-tight text-center" style={{ fontFamily: "var(--font-montserrat)" }}>
                      Tres modelos<br />financieros
                    </p>
                  </div>
                  {/* Lock overlay */}
                  <div className="absolute inset-0 backdrop-blur-sm bg-[#0E1133]/70 flex flex-col items-center justify-center gap-4">
                    <div
                      ...
                      style={{ background: "rgba(45,182,193,.15)", border: "1.5px solid rgba(45,182,193,.35)" }}
                    >
                      <svg viewBox="0 0 24 24" fill="currentColor" className="w-7 h-7 text-[#2DB6C1] ml-0.5">
                        <path d="M8 5.14v14l11-7-11-7z" />
                      </svg>
                    </div>
                    <div>
                      <p className="text-white font-bold text-sm mb-1" style={{ fontFamily: "var(--font-montserrat)" }}>
                        Contenido exclusivo para registrados
                      </p>
                ...
                <div className="mt-3 flex items-center justify-center gap-2">
                  {["MR", "LC", "JH", "AP", "CV"].map((l, i) => (
                    <div
                      key={l}
                      className="w-6 h-6 rounded-full text-[8px] font-black flex items-center justify-center border-2 border-[#0E1133]"
                      style={{
                        background: ["#2DB6C1","#5A9EDB","#7C6EDB","#4FC89A","#E8956B"][i],
                        color: i === 0 || i === 3 ? "#0E1133" : "#fff",
              ...
              <h2
                className="font-black text-2xl text-white mb-3 leading-tight"
                style={{ fontFamily: "var(--font-montserrat)", letterSpacing: "-0.5px" }}
              >
                Regístrate para ver el webinar completo
              </h2>
              <p className="text-white/50 text-sm mb-6 max-w-sm mx-auto leading-relaxed">
                Acceso gratuito e inmediato. Incluye chat en vivo con otros participantes.
              </p>
              <a
                href="#registro"
                className="inline-flex items-center gap-2 px-7 py-3.5 rounded-xl font-black text-sm tracking-wide transition-all hover:-translate-y-0.5"
                style={{ background: "#2DB6C1", color: "#0E1133", fontFamily: "var(--font-montserrat)" }}
              >
              ...
              <div className="mt-10 flex flex-wrap gap-8 justify-center">
                {[
                  { icon: "🎬", label: "1h 20min de contenido" },
                  { icon: "👥", label: "+200 profesionales registrados" },
                  { icon: "📋", label: "Guía de cumplimiento incluida" },
                ].map((s) => (
      ...
      <section id="speakers" className="hp-section hp-bg-1 py-20 px-4 sm:px-6 lg:px-8" style={{ borderTop: "1px solid var(--hp-divider)", borderBottom: "1px solid var(--hp-divider)" }}>
        <div className="max-w-6xl mx-auto">
          ...
          <h2
            ...
            style={{ fontFamily: "var(--font-montserrat), Montserrat, sans-serif", fontSize: "clamp(28px,3vw,40px)", color: "var(--hp-text-hi)", letterSpacing: "-1px" }}
          >
          ...
          <div className="grid md:grid-cols-2 gap-7">
            ...
            <div className="rounded-2xl overflow-hidden" style={{ border: "1px solid var(--hp-card-border)" }}>
              <div className="hp-bg-hero relative overflow-hidden p-6 pb-5">
                ...
                <div className="relative z-10 flex items-end gap-4 mb-4">
                  <SpeakerAvatar src="/images/webinar/arianna-quezada.jpg" fallback="AQ" gradient="linear-gradient(135deg,#25969f,#2DB6C1)" />
                  <div>
                    <div className="text-lg font-black leading-tight text-white mb-0.5" style={{ fontFamily: "var(--font-montserrat), Montserrat, sans-serif" }}>Arianna Quezada</div>
                ...
                <div className="inline-flex items-center gap-1.5 rounded-full px-3 py-1" style={{ background: "rgba(45,182,193,.12)", border: "1px solid rgba(45,182,193,.25)" }}>
                  ...
                  <span className="text-[10px] font-bold text-[#2DB6C1] tracking-wide" style={{ fontFamily: "var(--font-montserrat), Montserrat, sans-serif" }}>8 años liderando JAAK</span>
                </div>
              </div>
              <div className="p-6" style={{ background: "var(--hp-card-bg)" }}>
                <Accordion highlights={ariannaHighlights} open={openA} onToggle={(i) => setOpenA(openA === i ? null : i)} />
              </div>
            </div>
            ...
            <div className="rounded-2xl overflow-hidden" style={{ border: "1px solid var(--hp-card-border)" }}>
              <div className="hp-bg-hero relative overflow-hidden p-6 pb-5">
                <div className="absolute inset-0 pointer-events-none" style={{ backgroundImage: "linear-gradient(rgba(45,182,193,.05) 1px,transparent 1px),linear-gradient(90deg,rgba(45,182,193,.05) 1px,transparent 1px)", backgroundSize: "28px 28px" }} />
                <div className="relative z-10 flex items-end gap-4 mb-4">
                  <SpeakerAvatar src="/images/webinar/david-merino.jpg" fallback="DM" gradient="linear-gradient(135deg,#142654,#1E3A7A)" />
                  <div>
                    <div className="text-lg font-black leading-tight text-white mb-0.5" style={{ fontFamily: "var(--font-montserrat), Montserrat, sans-serif" }}>Dr. David Merino</div>
                    <div className="text-[12px] text-white/50">Especialista en Compliance · Contrainteligencia Empresarial</div>
                  </div>
                </div>
                <div className="inline-flex items-center gap-1.5 rounded-full px-3 py-1" style={{ background: "rgba(45,182,193,.12)", border: "1px solid rgba(45,182,193,.25)" }}>
                  <div className="w-1.5 h-1.5 rounded-full bg-[#2DB6C1]" />
                  <span className="text-[10px] font-bold text-[#2DB6C1] tracking-wide" style={{ fontFamily: "var(--font-montserrat), Montserrat, sans-serif" }}>Entrenado por ONU · GAFI · FBI</span>
                </div>
              </div>
              <div className="p-6" style={{ background: "var(--hp-card-bg)" }}>
                <Accordion highlights={davidHighlights} open={openD} onToggle={(i) => setOpenD(openD === i ? null : i)} />
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="flex flex-wrap gap-2">
              {certs.map((c) => (
                <span
                  ...
                  style={{ fontFamily: "var(--font-montserrat), Montserrat, sans-serif", color: "rgba(45,182,193,.75)", background: "rgba(45,182,193,.07)", border: "1px solid rgba(45,182,193,.18)" }}
                >
                  {c}
                </span>
              ))}
          ...
          <div className="divide-y" style={{ borderColor: "var(--hp-divider)" }}>
            {steps.map((step) => (
              <div key={step.num} className="flex gap-4 py-5">
                <span className="text-[11px] font-bold shrink-0 pt-0.5 w-6" style={{ fontFamily: "var(--font-montserrat), Montserrat, sans-serif", color: "#2DB6C1", letterSpacing: ".08em" }}>
                  {step.num}
                </span>
                <div>
                  <div className="text-sm font-black mb-1" style={{ fontFamily: "var(--font-montserrat), Montserrat, sans-serif", color: "var(--hp-text-hi)" }}>
                    {step.title}
                  </div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {step.tags.map((t) => (
                      <span key={t} className="text-[9.5px] font-bold rounded px-2 py-0.5" style={{ fontFamily: "var(--font-montserrat), Montserrat, sans-serif", color: "rgba(45,182,193,.8)", background: "rgba(45,182,193,.08)", border: "1px solid rgba(45,182,193,.18)" }}>
                        {t}
                      </span>
                    ))}
                  </div>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {products.map((p) => (
              <div
                ...
                style={{ border: "1.5px solid var(--hp-card-border)", background: "var(--hp-card-bg)" }}
                onMouseEnter={(e) => {
              ...
              >
                <div className="w-11 h-11 rounded-xl flex items-center justify-center mb-4 text-[#2DB6C1]" style={{ background: "rgba(45,182,193,.1)", border: "1.5px solid rgba(45,182,193,.25)" }}>
                  {p.icon}
                </div>
                <h3 className="text-sm font-black mb-2 leading-snug" style={{ fontFamily: "var(--font-montserrat), Montserrat, sans-serif", color: "var(--hp-text-hi)" }}>
                  {p.name}
                </h3>
                <p className="text-xs leading-relaxed mb-3" style={{ color: "var(--hp-text-md)" }}>{p.desc}</p>
                <div className="flex flex-wrap gap-1 mb-4">
                  {p.tags.map((t) => (
                    <span key={t} className="text-[9.5px] font-semibold rounded px-2 py-0.5" style={{ color: "#2DB6C1", background: "rgba(45,182,193,.08)", borderRadius: "3px" }}>{t}</span>
                  ))}
                </div>
          ...
          <div className="mt-10 text-center">
            <a
              ...
              style={{ background: "#212A45", fontFamily: "var(--font-montserrat), Montserrat, sans-serif" }}
            >
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8 text-center" style={{ background: "linear-gradient(135deg,#25969f 0%,#2DB6C1 100%)" }}>
        <div className="max-w-2xl mx-auto">
          ...
          <a
            ...
            style={{ background: "#0E1133", fontFamily: "var(--font-montserrat), Montserrat, sans-serif" }}
          >
