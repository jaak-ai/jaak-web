...
```bash
curl -X POST https://signa.sandbox.jaak.ai/api/v1/templates \
...
```bash
curl -X POST https://signa.sandbox.jaak.ai/api/v1/submissions \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
  ...
  "signers": [
    {
      ...
      "email": "juan.perez@ejemplo.com",
      "status": "pending",
      "signUrl": "https://signa.sandbox.jaak.ai/sign/sub_def456?signer=firmante_1"
    }
  ],
...
```bash
curl -X GET https://signa.sandbox.jaak.ai/api/v1/submissions/sub_def456/download \
  -H "Authorization: Bearer $TOKEN" \
...
```bash
curl -X GET https://signa.sandbox.jaak.ai/api/v1/submissions/sub_def456 \
  -H "Authorization: Bearer $TOKEN"
```
