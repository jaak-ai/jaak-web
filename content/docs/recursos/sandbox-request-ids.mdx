...
> **Estado del servicio**: **Activo en sandbox** desde el **2026-04-17** (imagen `jaak-rest-dummy v0.4.1`). Endpoint operativo: `https://api.sandbox.jaak.ai/api/v1/kyc/sandbox/simulate`.
> **Nota sobre timeouts (§5.8)**: los prefijos `SET*` devuelven `HTTP 408` (Request Timeout) en lugar de `504` — el api-gateway trata `504` como fallo transitorio y hace retry + fallback al `nfury-api` real, lo que enmascara el escenario. `408` llega intacto al integrador.
> **Audiencia**: integradores (desarrolladores de clientes), QA, soporte, customer success.
> **Ambiente afectado**: `https://api.sandbox.jaak.ai` (namespace `api-dummy`).
...
- **No aplica** a producción (`services.api.jaak.ai` y `api.dev.jaak.ai`) — producción siempre usa la lógica real.
...
**Simular error 401 de licencia** (reproduce el bug que tuvimos en TO-154/TO-155):
...
El dispatcher **respeta el mismo shape** del endpoint en producción (`Flow` y `VerificationFlow` en `jaak-nfury-api`). Solo cambian los valores concretos según el escenario. Así tu parser del cliente puede integrarse sin ramas especiales para sandbox.
...
- Spec técnico (interno, equipo backend): [`jaak-ai/jaak-rest-dummy` — docs/kyc-sandbox-catalog.md](https://github.com/jaak-ai/jaak-rest-dummy/blob/main/docs/kyc-sandbox-catalog.md)
- PR del spec mergeado: [jaak-ai/jaak-rest-dummy#3](https://github.com/jaak-ai/jaak-rest-dummy/pull/3)
...
- Tareas internas origen: Tech Ops — TO-154 (Liveness 401), TO-155 (Document OCR 401).
...
| Campo | Valor |
|---|---|
| Estado | En desarrollo (spec aprobado, implementación pendiente) |
| Versión del catálogo | v0.1 (propuesta) |
| Última revisión | 2026-04-17 |
| Próxima actualización | Al publicar el dispatcher en sandbox (fecha TBD) |
| Contacto técnico interno | equipo backend (canal `#tech-dev`) |
