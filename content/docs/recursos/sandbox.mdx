...
- **Dashboard sandbox:** `https://platform.sandbox.jaak.ai/settings/api`
...
```bash
curl -X POST https://api.sandbox.jaak.ai/api/v1/sessions \
...
Cambia todas las URLs de `*.sandbox.jaak.ai` a `*.jaak.ai`:

```javascript
// Sandbox
const BASE_URL = 'https://api.sandbox.jaak.ai';
...
```bash
# .env.development
JAAK_API_URL=https://api.sandbox.jaak.ai
...
const config = {
  development: {
    apiUrl: 'https://api.sandbox.jaak.ai',
    apiKey: process.env.JAAK_API_KEY_DEV,
    apiSecret: process.env.JAAK_API_SECRET_DEV
  },
  staging: {
    apiUrl: 'https://api.sandbox.jaak.ai', // También usa sandbox
...
describe('JAAK Integration', () => {
  ...
  const baseUrl = 'https://api.sandbox.jaak.ai';
...
2. Confirma que las credenciales sean del dashboard de sandbox (`platform.sandbox.jaak.ai`)
